...
use crate::{event, poll, sys, Interest, Token};
...
impl RegistrationInner {
    ...
    fn update(
        ...
        if queue.is_null() {
            ...
            let actual = self
                .readiness_queue
                .compare_exchange(queue, other, Release, Relaxed)
                .unwrap_or_else(|e| e);
...
impl ReadinessQueue {
    ...
    pub(super) fn prepare_for_sleep(&self) -> bool {
        ...
        let actual = self
            .inner
            .head_readiness
            .compare_exchange(end_marker, sleep_marker, AcqRel, Relaxed)
            .unwrap_or_else(|e| e);
...
impl ReadinessQueueInner {
    ...
    fn enqueue_node(&self, node: &ReadinessNode) -> bool {
        ...
        unsafe {
            ...
            loop {
                ...
                let act = self
                    .head_readiness
                    .compare_exchange(prev, node_ptr, AcqRel, Relaxed)
                    .unwrap_or_else(|e| e);
    ...
    fn clear_sleep_marker(&self) {
        ...
        unsafe {
            ...
            let actual = self
                .head_readiness
                .compare_exchange(sleep_marker, end_marker, AcqRel, Relaxed)
                .unwrap_or_else(|e| e);
...
impl AtomicState {
    ...
    fn compare_and_swap(
        ...
        self.inner
            .compare_exchange(current.into(), new.into(), order, Relaxed)
            .unwrap_or_else(|e| e)
            .into()
    }
